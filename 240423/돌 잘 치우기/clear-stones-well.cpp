...
void combination(int m, int index, vector<pair<int, int>>& current, vector<vector<pair<int, int>>>& result, vector<pair<int, int>> rock)//돌 위치 정보 저장)
{
    if(current.size() == m)
    {
    ...
    for(int i=index;i<rock.size();i++)
    {
        ...
        combination(m,i+1, current, result,rock);
        current.pop_back();
    }
}
